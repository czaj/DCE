...
NVar = (~EstimOpt.FullCov)*(NVarA*2 + EstimOpt.NVarM*NVarA + EstimOpt.NVarS + EstimOpt.NVarNLT + 2*EstimOpt.Johnson)+...
    EstimOpt.FullCov*(NVarA*(1+EstimOpt.NVarM) + sum(1:NVarA) + EstimOpt.NVarS + EstimOpt.NVarNLT + 2*EstimOpt.Johnson);
...
if EstimOpt.FullCov == 0
    if exist('B_backup','var') && ~isempty(B_backup) && size(B_backup,1) == NVarA*2 + EstimOpt.NVarM*NVarA + EstimOpt.NVarS + EstimOpt.NVarNLT + 2*EstimOpt.Johnson
        b0 = B_backup(:);
    ...
    if  ~exist('b0','var')
        if isfield(Results_old,'MNL') && isfield(Results_old.MNL,'bhat') && length(Results_old.MNL.bhat) == (NVarA*(1+ EstimOpt.NVarM) + EstimOpt.NVarS + EstimOpt.NVarNLT) %+ 2*EstimOpt.Johnson)
            disp('Using MNL results as starting values')
            Results_old.MNL.bhat = Results_old.MNL.bhat(:);
            %             b0 = [Results_old.MNL.bhat(1:NVarA);max(1,sqrt(abs(Results_old.MNL.bhat(1:NVarA))));0.1*ones(EstimOpt.NVarM.*NVarA,1);Results_old.MNL.bhat(NVarA+1:end)];
...
if sum(EstimOpt.Dist == -1) > 0
    ...
    elseif EstimOpt.FullCov == 1
        ...
        %         EstimOpt.BActive(NVarA+1:NVarA+sum(1:NVarA)) = EstimOpt.BActive(NVarA+1:NVarA+sum(1:NVarA)) .* (Vt(find(tril(ones(size(Vt)))))');
...
if EstimOpt.FullCov == 0
    ...
    %    std_out = Results.std(NVarA+1:NVarA*2);
    %    std_out(imag(Results.std(NVarA+1:NVarA*2)) ~= 0) = NaN;
    %    Results.DetailsV = [Results.bhat(NVarA+1:NVarA*2).^2,2.*std_out.*abs(Results.bhat(NVarA+1:NVarA*2)),pv((Results.bhat(NVarA+1:NVarA*2)).^2,2.*std_out.*abs(Results.bhat(NVarA+1:NVarA*2)))];
...
elseif EstimOpt.FullCov == 1
    ...
    if EstimOpt.NVarNLT > 0
        ...
        %         for i=1:EstimOpt.NVarNLT; Results.DetailsNLT = [Results.DetailsNLT; [Results.bhat(NVarA*(2+EstimOpt.NVarM)+EstimOpt.NVarS+i),Results.std(NVarA*(2+EstimOpt.NVarM)+EstimOpt.NVarS+i),pv(Results.bhat(NVarA*(2+EstimOpt.NVarM)+EstimOpt.NVarS+i),Results.std(NVarA*(2+EstimOpt.NVarM)+EstimOpt.NVarS+i))]];end
        Results.DetailsNLT0 = NaN(NVarA,4);
        Results.DetailsNLT0(EstimOpt.NLTVariables,:) = Results.DetailsNLT;
        Results.R = [Results.R, Results.DetailsNLT0];
    end
    ...
    Results.chol = [Results.bhat(NVarA+1:NVarA*(NVarA/2+1.5)),Results.std(NVarA+1:NVarA*(NVarA/2+1.5)),pv(Results.bhat(NVarA+1:NVarA*(NVarA/2+1.5)),Results.std(NVarA+1:NVarA*(NVarA/2+1.5)))]; %Results.R = (1:NVarA*(NVarA/2+0.5),size(Results.R,2)+1:size(Results.R,2)+3) =
