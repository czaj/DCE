...
if nargout == 1 % function value only
    ...
    if any(isnan(Xa(:))) == 0  % faster version for complete dataset
        ...
        parfor n = 1:NP
            ...
            if WTP_space == 0
                X_hat = sum(reshape(U_prob.*Xa_n(YnanInd,:,ones(NRep,1)),[NAltMiss(n),NCTMiss(n),NVarA,NRep]),1);
    ...
    for i = 1:size(Xmea,2)
        ...
        elseif MeaSpecMatrix(i) == 2 % Ordered Probit
            ...
            if MeaExpMatrix(i) ~= 0
                gmea(:,:,l+1+sum(MeaMatrix(:,i)' == 1):l+sum(MeaMatrix(:,i)' == 1)+tmp) = -grad_tmp.*Xmea_exp_expand ; % additional explanatory variables of Xmea
            end
    ...
    g3 = reshape(mean(probs.*gmea,2),[NP,size(bmea,1)]); % NP x NVarmea
