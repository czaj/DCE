...
%             U = reshape(Xa(~isnan(Y(:,n)),:,n)*b_mtx(:,((n-1)*EstimOpt.NRep+1):n*EstimOpt.NRep),EstimOpt.NAlt,EstimOpt.NCT-sum(isnan(Y(1:EstimOpt.NAlt:end,n))),EstimOpt.NRep);
            U = reshape(Xa(YnanInd,:,n)*b_mtx(:,((n-1)*EstimOpt.NRep+1):n*EstimOpt.NRep),EstimOpt.NAltMiss(n),EstimOpt.NCTMiss(n),EstimOpt.NRep);
            U_max = max(U);
%             U = exp(U - U_max(ones(EstimOpt.NAlt,1),:,:)); % NAlt x NCT - NaNs x NRep
            U = exp(U - U_max(ones(EstimOpt.NAltMiss(n),1),:,:)); % NAlt x NCT - NaNs x NRep
            U_sum = reshape(sum(U,1),EstimOpt.NCTMiss(n),EstimOpt.NRep);
            U_selected = reshape(U(Y(YnanInd ,n*ones(EstimOpt.NRep,1))==1),EstimOpt.NCTMiss(n),EstimOpt.NRep);   
            probs(n,:) = prod(U_selected ./ U_sum,1);
        end;
    end 
    ...
    if any(isnan(Xa(:))) == 0  % faster version for complete dataset
        ...
        parfor n = 1:EstimOpt.NP
            ...
            if EstimOpt.WTP_space == 0   
                ...
                sumFsqueezed(EstimOpt.MNLDist ==1, :) = sumFsqueezed(EstimOpt.MNLDist ==1, :).*b_mtx(EstimOpt.MNLDist ==1,((n-1)*EstimOpt.NRep+1):n*EstimOpt.NRep);
                ...
                sumFsqueezed(EstimOpt.MNLDist ==1, :) = sumFsqueezed(EstimOpt.MNLDist ==1, :).*b_mtx_grad(EstimOpt.MNLDist ==1,:,n);
                sumFsqueezed_LV = sumFsqueezed'*bl; % NRep x NLatent
            end
        ...
        parfor n = 1:EstimOpt.NP
            YnanInd = ~isnan(Y(:,n));
            U = reshape(Xa(YnanInd,:,n)*b_mtx(:,((n-1)*EstimOpt.NRep+1):n*EstimOpt.NRep),EstimOpt.NAltMiss(n),EstimOpt.NCTMiss(n),EstimOpt.NRep); % NAlt x NCT x NRep
            U_max = max(U);
            U = exp(U - U_max(ones(EstimOpt.NAltMiss(n),1),:,:)); % rescale utility to avoid exploding 
            U_sum = reshape(sum(U,1),1,EstimOpt.NCTMiss(n),EstimOpt.NRep); 
            U_prob = U./U_sum(ones(EstimOpt.NAltMiss(n),1,1),:,:); % NAlt x NCT x NRep
            probs(n,:) = prod(reshape(U_prob(Y(YnanInd,n*ones(EstimOpt.NRep,1))==1),EstimOpt.NCTMiss(n),EstimOpt.NRep),1); % 1 x NRep
            %probs(n,:) = U_prod;    
        
            % calculations for gradient
            U_prob = reshape(U_prob, EstimOpt.NAltMiss(n)*EstimOpt.NCTMiss(n),1, EstimOpt.NRep); % NAlt*NCT x NVarA x NRep
            if EstimOpt.WTP_space == 0   
                X_hat = sum(reshape(U_prob(:,ones(1,EstimOpt.NVarA,1),:).* Xa(YnanInd,:, n*ones(EstimOpt.NRep,1)), EstimOpt.NAltMiss(n), EstimOpt.NCTMiss(n), EstimOpt.NVarA, EstimOpt.NRep),1);
                if EstimOpt.NCTMiss(n) ~= 1
                    F = Xa(Y(:,n) == 1,:,n*ones(EstimOpt.NRep,1)) - squeeze(X_hat); %NCT x NVarA x NRep 
                    sumFsqueezed = squeeze(sum(F,1)); %NVarA x NRep                    
                else
                    sumFsqueezed = squeeze(Xa(Y(:,n) == 1,:,n*ones(EstimOpt.NRep,1))) - squeeze(X_hat); %NVarA x NRep 
                ...
                b_mtx_wtp = reshape(b_mtx(:,((n-1)*EstimOpt.NRep+1):n*EstimOpt.NRep), 1, EstimOpt.NVarA, EstimOpt.NRep);
                Xalpha = Xa(YnanInd,1:end-EstimOpt.WTP_space, n*ones(EstimOpt.NRep,1)).*b_mtx_wtp(ones(EstimOpt.NAltMiss(n)*EstimOpt.NCTMiss(n),1),EstimOpt.WTP_matrix,:);
                % for non-cost variables
                X_hat1 = sum(reshape(U_prob(:,ones(1,EstimOpt.NVarA-EstimOpt.WTP_space),:).* Xalpha, EstimOpt.NAltMiss(n), EstimOpt.NCTMiss(n), EstimOpt.NVarA-EstimOpt.WTP_space, EstimOpt.NRep),1);
                F1 = Xalpha(Y(YnanInd,n) == 1,:,:) - reshape(X_hat1, EstimOpt.NCTMiss(n), EstimOpt.NVarA-EstimOpt.WTP_space, EstimOpt.NRep); %NCT x NVarA-WTP_space x NRep   
                % for cost variables
                if EstimOpt.WTP_space == 1 % without starting the loop
                    ...
                    X_hat2 = sum(reshape(squeeze(U_prob).*pX, EstimOpt.NAltMiss(n), EstimOpt.NCTMiss(n), EstimOpt.WTP_space, EstimOpt.NRep),1);
                    F2 = pX(Y(YnanInd,n) == 1,:,:) - reshape(X_hat2, EstimOpt.NCTMiss(n), EstimOpt.NRep); %NCT x NRep    
                else
                    pX = zeros(EstimOpt.NCTMiss(n)*EstimOpt.NAltMiss(n), EstimOpt.WTP_space, EstimOpt.NRep);
                    for i = 1:EstimOpt.WTP_space
                    ...
                    X_hat2 = sum(reshape(U_prob(:,ones(1,EstimOpt.WTP_space),:).* pX, EstimOpt.NAltMiss(n), EstimOpt.NCTMiss(n), EstimOpt.WTP_space, EstimOpt.NRep),1);
                    F2 = pX(Y(YnanInd,n) == 1,:,:) - reshape(X_hat2, EstimOpt.NCTMiss(n),EstimOpt.WTP_space, EstimOpt.NRep); %NCT x WTP x NRep 
                end
