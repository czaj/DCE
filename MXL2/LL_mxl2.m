function [f,g] = LL_mxl2(YY,XXa,XXm,Xs,err,EstimOpt,b0)
...
%             %p0(n) = max(mean(U_prod),realmin);
            ...
            U_prob(isinf(U_prob))  =realmax*sign(U_prob(isinf(U_prob)));
            U_sum(isinf(U_sum))  =realmax*sign(U_sum(isinf(U_sum)));
            if WTP_space == 0   
                X_hat = sum(reshape(bsxfun(@times,U_prob,XXa_n), NAlt-1, NCT, NVarA, NRep),1); % 1 x NCT x NVarA x NRep
                if NCT ~= 1
                    X_hat(isinf(X_hat))  =realmax*sign(X_hat(isinf(X_hat)));
            ...
            sumFsqueezed(isinf(sumFsqueezed))  =realmax*sign(sumFsqueezed(isinf(sumFsqueezed)));
            if NVarS >0
                if WTP_space == 0
            ...
            if FullCov == 0
                ...
                sumVC2tmp(isinf(sumVC2tmp))  =realmax*sign(sumVC2tmp(isinf(sumVC2tmp)));
