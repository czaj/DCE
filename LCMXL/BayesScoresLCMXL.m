...
PClass = PClass./PClass_sum(:,ones(NClass,1)); ... % class probabilities (prior)
fx = sum(p.*PClass,2); ...
PClass_i = reshape(p.*PClass./fx(:,ones(NClass,1)),[1,NP,NClass]); ... class probabilities (posterior)
...
Betas_i = sum(squeeze(sum(bsxfun(@times,bsxfun(@rdivide,p_tmp,sum(p_tmp,2)),b_mtx_sliced),2)).*PClass2(ones(NVarA,1,1),:,:),3)';
