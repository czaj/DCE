...
% LL_hmxl_MATlike MATLAB-optimizer wrapper for LL_hmxl.
%
...
%   - guards against absurd parameter values before LL_hmxl is called.

global HMXL_OPT_STATE

if nargin < 11
    error('LL_hmxl_MATlike:NotEnoughInputs','Not enough inputs.');
end
...
if isempty(HMXL_OPT_STATE) || ~isstruct(HMXL_OPT_STATE)
    HMXL_OPT_STATE = struct();
end
if ~isfield(HMXL_OPT_STATE,'eval_count') || isempty(HMXL_OPT_STATE.eval_count)
    HMXL_OPT_STATE.eval_count = 0;
end
HMXL_OPT_STATE.eval_count = HMXL_OPT_STATE.eval_count + 1;
...
evalTimer = tic;
if EstimOpt.DebugOptimizationEval ~= 0 && (mod(HMXL_OPT_STATE.eval_count-1,EstimOpt.DebugEvalEvery) == 0)
    local_progress_msg(sprintf('HMXL eval %d started: max|B|=%.4g, NRep=%d.',HMXL_OPT_STATE.eval_count,max(abs(b0)),EstimOpt.NRep));
end
...
if any(~isfinite(b0)) || maxAbsB > EstimOpt.MaxAbsB
    ...
    if EstimOpt.ReportOptimizationDiagnostics ~= 0
        local_warn_once(sprintf('HMXL objective penalty: parameter %d reached %.4g (limit %.4g).',idxMaxB,maxAbsB,EstimOpt.MaxAbsB),EstimOpt);
    end
    return
end
...
try
    if isequal(OptimOpt.GradObj,'on')
        ...
        if length(EstimOpt.BActive) ~= npar
            error('LL_hmxl_MATlike:BActiveSize','EstimOpt.BActive has incorrect length.');
        end
    ...
    if EstimOpt.RealMin >= 2
        ...
        if EstimOpt.ReportOptimizationDiagnostics ~= 0
            local_warn_once(['HMXL objective penalty after caught error: ' ME.message],EstimOpt);
        end
        return
    else
        rethrow(ME)
...
if EstimOpt.DebugOptimizationEval ~= 0 && (mod(HMXL_OPT_STATE.eval_count-1,EstimOpt.DebugEvalEvery) == 0)
    local_progress_msg(sprintf('HMXL eval %d finished in %.2fs: LL=%.6g, max|g|=%.4g.',HMXL_OPT_STATE.eval_count,diagInfo.elapsed_seconds,LL,max(abs(g))));
end
...
global HMXL_OPT_STATE
HMXL_OPT_STATE.last_B = b(:);
HMXL_OPT_STATE.last_LL = LL;
HMXL_OPT_STATE.last_g = g;
HMXL_OPT_STATE.last_status = status;
HMXL_OPT_STATE.last_diag = diagInfo;
if isfinite(LL) && all(isfinite(g))
    if ~isfield(HMXL_OPT_STATE,'best_LL') || isempty(HMXL_OPT_STATE.best_LL) || LL < HMXL_OPT_STATE.best_LL
        HMXL_OPT_STATE.best_LL = LL;
        HMXL_OPT_STATE.best_B = b(:);
        HMXL_OPT_STATE.best_g = g;
        HMXL_OPT_STATE.best_status = status;
        HMXL_OPT_STATE.best_diag = diagInfo;
    end
end
...
if EstimOpt.ReportOptimizationDiagnostics ~= 0
    ...
    if bad
        msg = sprintf('HMXL diagnostic: status=%s, LL=%.6g, nonfinite f=%d, nonfinite jac=%d, max|jac|=%.4g, max|B|=%.4g.', ...
...
global HMXL_OPT_STATE
if ~isfield(HMXL_OPT_STATE,'warning_count') || isempty(HMXL_OPT_STATE.warning_count)
    HMXL_OPT_STATE.warning_count = 0;
end
if HMXL_OPT_STATE.warning_count < EstimOpt.MaxDiagnosticWarnings
    HMXL_OPT_STATE.warning_count = HMXL_OPT_STATE.warning_count + 1;
    try
        fprintf(2,'WARNING: %s\n',msg);
    catch
        warning('HMXL:OptimizationDiagnostic','%s',msg);
    end
end
end
