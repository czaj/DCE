function [f,g] = LL_hmxl(Y,Xa,Xm,X_str,X_mea,Xmea_exp,err_sliced,EstimOpt,B)
...
%f = -log(max(realmin,mean(probs.*L_mea,2)));
    ...
    LV_std = sum(LV_tmp(:,:,ones(EstimOpt.NVarstr,1)).*X_str_expand,2)/(EstimOpt.NRep*EstimOpt.NP-1);% Latent x 1 x NVarstr
    ...
    LV_der = reshape(X_str_expand - LV_tmp.*LV_std(:,ones(EstimOpt.NRep*EstimOpt.NP,1),:), EstimOpt.NLatent, EstimOpt.NRep, EstimOpt.NP, EstimOpt.NVarstr); % Latent x NRep x NP x NVarstr
